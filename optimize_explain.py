    ...
    if wrong_part:
        ...
        for wrong_answer in wrong_answers:
            if wrong_answer.strip():
                ...
                match = re.match(r'\*\*(.+?)\*\*:\s*(.*?)\s*→\s*(.*?)`(.+?)`\s*=\s*(.*?)\s*→\s*(.*)', wrong_answer)
                if match:
                    answer = match.group(1)
                    meaning = match.group(2).strip()
                    emoji_reason = match.group(3).strip()
                    pattern = match.group(4)
                    pattern_explain = match.group(5).strip()
                    final_reason = match.group(6).strip()
                    
                    html += f'    <div style="margin-bottom: 12px; padding: 10px; background-color: white; border-left: 3px solid #ff6b6b;">\n'
...
for item in data:
    if item.get('explain'):
        ...
        # Chúng ta sẽ chuyển sang HTML đơn giản hơn
        ...
        # Replace headers
        new_explain = new_explain.replace('### ✅ Đáp án đúng', '<h3 style="color: #27ae60; border-bottom: 2px solid #27ae60; padding-bottom: 5px;">✅ Đáp án đúng</h3>')
        new_explain = new_explain.replace('### ❌ Các đáp án sai', '<h3 style="color: #e74c3c; border-bottom: 2px solid #e74c3c; padding-bottom: 5px; margin-top: 20px;">❌ Các đáp án sai</h3>')
...
print(f"   - Chuyển đổi markdown bold sang HTML <strong>")
print(f"   - Cung cấp màu sắc cho đáp án đúng/sai")
print(f"   - Cải thiện định dạng text")
