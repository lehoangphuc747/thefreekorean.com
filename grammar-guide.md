...
- **N + 이다**: gắn trực tiếp vào danh từ để tạo vị ngữ (không có khoảng trắng).
- **Biến thể theo phong cách/nối âm**:
  - **Trang trọng (격식체)**: `N입니다`.
  - **Lịch sự thân mật (해요체)**:
    - Nếu danh từ có 받침: `N이에요`.
    - Nếu danh từ không 받침: `N예요`.
  - **Thân mật (반말)**:
    - Nếu danh từ có 받침: `N이야`.
    - Nếu danh từ không 받침: `N야`.
- **Quá khứ với ‘-었-’**:
  - Nếu danh từ có 받침: `N이었-` → 예) 선생님이었어요.
  - Nếu danh từ không 받침: `N였-` (rút gọn của 이었-) → 예) 의사였어요.
- **Kết hợp với ‘-아서/어서’ (lí do)**:
  - `이어서/이라서` (chuẩn).
  - Không 받침 có thể rút gọn: `이어서 → 여서`, `이라서 → 라서`.

### Trật tự

- **Mẫu cơ bản**: `N1(주어/주제)은/는/이/가 + N2 + 이다`.
  - Ví dụ: `마이클은 미국 사람이다/입니다/이에요`.
- **Câu hỏi–trả lời điển hình**:
  - Hỏi: `이것이 무엇입니까?/뭐예요?`
  - Đáp: `이것은 N입니다/이에요/예요`.
- **Ngữ nghĩa thường gặp**: tên, nghề nghiệp, quốc tịch, giá cả, nơi chốn, v.v.
  - Ví dụ: `제 이름은 미도리입니다`, `한국의 수도는 서울입니다`, `하나에 이천 원이에요`.
